...
def reset_id_counter(start: int = 0) -> None:
    """Reset the module-global Chakra node id counter.

    Used so two trace-generation runs within the same Python process (e.g. a
    test suite that imports the module twice) produce consistent, isolated
    node IDs. Safe to call at any time; existing nodes already created are
    not affected, but new nodes from subsequent ``next_id()`` calls will
    start from ``start``.
    """
    global node_id
    node_id = int(start)
